...
import AddPageModal from "../../../components/Modals/AddPage";
...
const ContentPage = () => {
  ...
  const { pages, loading, totalCount } = useSelector((state) => state.Pages);
  ...
  const column = [
    ...
    },
    {
      ...
      isComponent: true,
      component: (item, ind) => (
        <span className="text-capitalize cursor-pointer">{item.slug}</span>
      ),
    },
    {
      head: "Title",
    ...
    },
    {
      ...
      isComponent: true,
      component: (item, ind) => (
        <span className="text-capitalize cursor-pointer">{"Description"}</span>
      ),
      ...
      //       <span>{isExpanded ? sanitizedContent : truncatedText}</span>
      //       {sanitizedContent.length > 100 && (
      //         <button
    ...
    //           ? "greenStatus"
    //           : ""
    //       } status m-0 rounded-pill px-3 text-capitalize py-1 fw-sbold`}
    //     >
    //       {item.status}
    //     </p>
    //   ),
  ...
  return (
    <>
      ...
      <AddPageModal
        ...
        title={`${
          formData.hasOwnProperty("_id") ? "Edit" : "Add"
        } content page`}
        content={""}
