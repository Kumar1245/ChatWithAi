...
import {
  ...
  editTokenPackage,
  getToken,
} from "../../../store/tokenpackage/actions";
import AddTokenModal from "../../../components/Modals/AddTokenModal";
import FadeLoader from "react-spinners/FadeLoader";
const TokenPackage = () => {
  const dispatch = useDispatch();
  const [currentPage, setCurrentPage] = useState(1);
  const { TOKEN, loading, totalCount } = useSelector((state) => state.Token);
  ...
  const fetchData = async () => {
    try {
      ...
      dispatch(getToken({ data: filter, callback }));
    } catch (error) {
        ...
        return (
          <>
            <div className="d-flex align-items-center gap-10">
              <div className="content">
                <p className="m-0 text-muted">
                  {item?.costPerToken ? "$ " + item?.costPerToken : ""}
                </p>
              </div>
            </div>
          </>
      ...
      component: (item, key) => {
        return (
          <>
            <div className="d-flex align-items-center gap-10">
              <div className="content">
                <p className="m-0 text-muted">{item.additionalWords}</p>
              </div>
            </div>
          </>
        );
  ...
  return (
    <>
      ...
      <AddTokenModal
        ...
        title={`${formData.hasOwnProperty("_id") ? "Edit" : "Add"} Token`}
        content={""}
