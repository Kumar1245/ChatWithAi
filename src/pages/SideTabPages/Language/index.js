...
import AddLanguageModal from "../../../components/Modals/LanguageModal";
const Language = () => {
  const dispatch = useDispatch();
  const [pageSize, setPageSize] = useState(10);
  const [tokenData, setTokenData] = useState(null);
  const [addModalShow, setAddModalShow] = useState();
  const { LANGUAGE, loading, totalCount } = useSelector(
  ...
  const fetchData = async () => {
    try {
      ...
      dispatch(getLanguage({ data: filter, callback }));
    } catch (error) {
  ...
  useEffect(() => {
    const languageOptionsArray = ISO6391.getAllCodes().map((code) => ({
      value: code,
      label: `${code.toUpperCase()} - ${ISO6391.getName(code)}`,
    }));
    inputFields.find((field) => field.type === "dynamic_select").options =
      languageOptionsArray;
  }, []);
  const clearFilter = () => {
    setFilter({ pageNo: 1, limit: pageSize, search: "" });
  };
          ...
          <Row>
            <Col lg="12" className="my-2">
              <div className="d-flex align-items-center justify-content-between gap-10">
                <h5 className="mb-0 fw-sbold text-capitalize">Language</h5>
                <Link
            ...
            <Col lg="12" className="my-2">
              {!loading && (
                <div className="cardCstm  rounded bg-white">
                  {!loading && LANGUAGE?.data?.length > 0 ? (
                    <>
                      <TableLayout
                        data={LANGUAGE?.data}
                        column={column}
